...
class RedirectCheckerTest extends UnitTestCase {
  ...
  public function testCanRedirect() {
    ...
    $checker = new RedirectChecker($this->getConfigFactoryStub($config), $state, $access, $account, $route_provider, $stream_wrapper_manager);
    ...
    $request->expects($this->any())
      ->method('getPathInfo')
      ->willReturn('/sites/default/files/css/css_test.css');
    $assetStream = $this->createMock('Drupal\Core\StreamWrapper\LocalStream');
    $assetStream->expects($this->any())
    ...
    $stream_wrapper_manager->expects($this->any())
      ->method('getViaScheme')
          ->with('assets')
          ->willReturn($assetStream);
    $checker = new RedirectChecker($this->getConfigFactoryStub($config), $state, $access, $account, $route_provider, $stream_wrapper_manager);
    $this->assertFalse($checker->canRedirect($request), 'Cannot redirect if a css asset file is requested');
