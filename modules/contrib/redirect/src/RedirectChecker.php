...
class RedirectChecker {
  ...
  public function canRedirect(Request $request, $route_name = NULL) {
    ...
    $assetsStreamWrapper = $this->streamWrapperManager->getViaScheme('assets');
    if (isset($route_name)) {
    ...
    elseif ($assetsStreamWrapper && preg_match('/^\/'  .  preg_quote($assetsStreamWrapper->getDirectoryPath(), '/') . '\/(css|js)\/.*/', $request_path_info)) {
      $can_redirect = FALSE;
    }
