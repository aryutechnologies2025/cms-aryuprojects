...
use Drupal\Core\Url;
use Drupal\sprowt_settings\ArrayUtil;
use Drupal\webform\Entity\Webform;
use Drupal\webform\Entity\WebformSubmission;
use Symfony\Component\HttpFoundation\RedirectResponse;

/**
 * Provides a Sprowt address autocomplete form.
 */
final class SwitchWebformAddressFieldForm extends FormBase
{
    ...
    {
        return 'sprowt_address_autocomplete_switch_webform_address_field';
    }
    ...
    public static function hasAddressField(Webform $webform) {
        ...
        foreach($elements as $key => $elementInfo) {
            ...
            if($elementInfo['#type'] == 'webform_address') {
    ...
    public function buildForm(array $form, FormStateInterface $form_state): array
    {
        ...
        $form['description'] = [
            '#type' => 'markup',
            '#markup' => Markup::create('<p>This will switch all "<strong>Webform address</strong>" fields on this form for "<strong>Sprowt address autocomplete</strong>" fields.</p>')
        ];
    ...
    public static function batchAddField($webformId, &$context) {
        ...
        $stub = [
            '#type' => 'sprowt_address_autocomplete',
            '#hidden_value__title_display' => 'invisible',
            '#requireFound' => true
        ];
        $keyMap = [];
        $stateElements = [];
        foreach($elements as $key => $element) {
            if($element['#type'] == 'webform_address') {
                $cloneRawElements = $rawElements;
                $rawElement = ArrayUtil::getValue($cloneRawElements, $element["#webform_parents"]);
                $stateElements[$key] = $rawElement;
                $newKey = $key . '_autocomplete';
                $newElement = $stub;
                $newElement['#address__title_display'] = $element['#address__title_display'] ?? 'before';
                $newElement['#title'] = !empty($element['#title']) ? $element['#title'] . ' (autocomplete)' : 'Address (autocomplete)';
                $newElement['#required'] = !empty($element['#required']);
                if(!empty($element['#address__title'])) {
                    $newElement['#address__title'] = $element['#address__title'];
                }
                $newElement['#address__required'] = $element['#address__required'] ?? !empty($element['#required']);
                \Drupal\sprowt_settings\ArrayUtil::insertAfterKey($rawElements, $element["#webform_parents"], $newElement, $newKey);
                $keyMap[$key] = $newKey;
            }
        }
        $elementStateKey = 'sprowt_address_autocomplete.webform_switch_elements.' . $webformId;
        \Drupal::state()->set($elementStateKey, $stateElements);
        $webform->setElements($rawElements);
        $webform->save();
        $context['results']['keyMap'] = $keyMap;
        $keymapStateKey = 'sprowt_address_autocomplete.webform_switch_keymap.' . $webformId;
        \Drupal::state()->set($keymapStateKey, $keyMap);
    ...
    public static function batchUpdateHandler($webformId, $handlerId, &$context)
    {
        ...
        $keyMap = $context['results']['keyMap'] ?? [];
        \Drupal::moduleHandler()->invokeAll('sprowt_address_autocomplete_replace_webform_address_fields_handler_update',  [
            $webform,
            $handler,
            $keyMap
        ]);
    ...
    public static function batchFinished($success, $results, $operations) {
        ...
        $webform = Webform::load($webformId);
        if($success) {
            \Drupal::messenger()->addStatus("Webform address field(s) switched");
        }
        else {
