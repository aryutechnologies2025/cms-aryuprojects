...
/**
 * Form controller for the context entity edit forms.
 */
final class ContextForm extends ContentEntityForm
{
    ...
    public function buildForm(array $form, FormStateInterface $form_state): array
    {
        ...
        if($form["content"]["widget"][0]['#type'] == 'text_format') {
            $form["content"]["widget"][0]['#format'] = 'code';
            $form["content"]["widget"][0]['#allowed_formats'] = ['code'];
        }


        $tagDescription = $form["tags"]["widget"]['#description'] ?? '';
        $vocab = Vocabulary::load(Context::$vocab);
    ...
    public function save(array $form, FormStateInterface $form_state): int
    {
        ...
        switch ($result) {
            case SAVED_NEW:
                $this->messenger()->addStatus($this->t('New context %label has been created.', $message_args));
                $this->logger('sprowt_ai')->notice('New context %label has been created.', $logger_args);
                break;

            case SAVED_UPDATED:
                $this->messenger()->addStatus($this->t('The context %label has been updated.', $message_args));
                $this->logger('sprowt_ai')->notice('The context %label has been updated.', $logger_args);
                break;
    ...
    public static function validateTags(array &$element, FormStateInterface $form_state)
    {
        ...
        foreach(array_keys($value) as $tagId) {
            ...
            if ($vid != Context::$vocab) {
                $form_state->setError($element, 'You can only add tags from the contexts vocabulary.');
            }
        }
