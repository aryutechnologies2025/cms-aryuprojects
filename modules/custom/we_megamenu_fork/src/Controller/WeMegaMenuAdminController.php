...
namespace Drupal\we_megamenu_fork\Controller;

use Drupal\Core\Url;
use Drupal\Core\Controller\ControllerBase;
use Drupal\system\Entity\Menu;
use Drupal\we_megamenu_fork\WeMegaMenuBuilder;
...
class WeMegaMenuAdminController extends ControllerBase {
  ...
  public function configWeMegaMenu($menu_name) {
    ...
    $build = [];
    $build['we_megamenu_fork'] = [
      '#theme' => 'we_megamenu_fork_backend',
      '#menu_name' => $menu_name,
      // '#items' => $tree,
    ...
    $build['we_megamenu_fork']['#attached']['library'][] = 'we_megamenu_fork/form.we-mega-menu-backend';
    $abs_url_save_config = Url::fromRoute('we_megamenu_fork.admin.save', [], ['absolute' => TRUE])->toString();
    $abs_url_reset_config = Url::fromRoute('we_megamenu_fork.admin.reset', [], ['absolute' => TRUE])->toString();
    $abs_url_icons_config = Url::fromRoute('we_megamenu_fork.geticons', [], ['absolute' => TRUE])->toString();
  ...
  public function saveConfigWeMegaMenu() {
    if (isset($_POST['action']) && $_POST['action'] == 'save') {
      ...
      we_megamenu_fork_flush_render_cache();
    }
    exit;
  }
  ...
  public function resetConfigWeMegaMenu() {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'reset-to-default' && isset($_POST['menu_name']) && isset($_POST['theme'])) {
      $query = \Drupal::database()->delete('we_megamenu_fork');
  ...
  public function styleOfBackendWeMegaMenu() {
    if (isset($_POST['type'])) {
      \Drupal::state()->set('we_megamenu_fork_backend_style', $_POST['type']);
      we_megamenu_fork_flush_render_cache();
    }
    exit;
  }
  ...
  public function listWeMegaMenus() {
    ...
    foreach ($menus as $name => $title) {
      ...
      $dropbuttons = [
        ...
        '#links' => [
          'config' => [
            'url' => new Url('we_megamenu_fork.admin.configure', ['menu_name' => $name]),
            'title' => 'Config',
          ],
          'edit' => [
    ...
    return [
      ...
      '#attributes' => ['id' => 'we_megamenu_fork'],
    ];
  }
  ...
  public function getIcons() {
    $file = DRUPAL_ROOT . '/' . $this->moduleHandler->getModule('we_megamenu_fork')->getPath() . '/assets/resources/icon.wemegamenu';
    $fh = fopen($file, 'r');
    $result = [];
    while ($line = fgets($fh)) {
      $result[] = trim($line);
