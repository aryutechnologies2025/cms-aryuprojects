...
namespace Drupal\we_megamenu_fork;
...
class WeMegaMenuBuilder {
  /**
   * Get menu tree we_megamenu_fork.
   *
  ...
  */
  ...
  public static function getAllBlocks() {
    ...
    if (empty($_list_blocks_array)) {
      ...
      /**
       * @var \Drupal\Core\Plugin\Context\LazyContextRepository $contextRepository
       */
      $contextRepository = \Drupal::service('context.repository');

      // Only add blocks which work without any available context.
      $definitions = $blockManager->getFilteredDefinitions('we_megamenu_fork', $contextRepository->getAvailableContexts());
      ...
      foreach ($definitions as $block_id => $block) {
        $provider_exceptions = ['we_megamenu_fork', 'layout_builder'];
        if (!in_array($block['provider'], $provider_exceptions)) {
          if ($block['category'] instanceof TranslatableMarkup) {
            $_list_blocks_array[$block['category']->__toString()][$block_id] = $block['admin_label'];
          }
          else {
            $_list_blocks_array[$block['category']][$block_id] = $block['admin_label'];
          }
        }
      }
    ...
    if ($bid && !empty($bid)) {
      ...
      if (isset($block) && !empty($block)) {
        ...
        if ($plugin_block instanceof Broken) {
          return $html .= '<p><b>Warning:</b> <i>Broken/Missing block</i></p>';
        }
  ...
  public static function renderWeMegaMenuBlock($menu_name, $theme) {
    return [
      '#theme' => 'we_megamenu_fork_frontend',
      '#block_theme' => $theme,
      '#menu_name' => $menu_name,
      '#section' => 'admin',
  ...
  public static function loadConfig($menu_name = '', $theme = '') {
    if (!empty($menu_name) && !empty($theme)) {
      $query = Drupal::database()->select('we_megamenu_fork', 'km');
                    ...
                    if (is_array($cols) && isset(reset($cols)->mlid)) {
                      ...
                      foreach ($cols as $key_col => $col) {
                        ...
                        if (is_array($menu_item)) {
                          ...
                          if (!empty($menu_item) && method_exists($menu_item, 'get')) {
                            $list_menu_items[] =  [
          ...
          if (isset($menu_item->rows_content)) {
            ...
            if (count($rows_content)) {
                    ...
                    foreach ($childs as $key_child => $child_uuid) {
                      ...
                      if (!in_array($child_uuid, $list_mega_items)) {
                        ...
                        $list_mega_items[] = $child_uuid;
                          //hopefully this fixes a bug? I'm not sure what's happening here
                          if(empty($menu_config->menu_config->{$key_menu}->rows_content[$row_count][$col_count])) {
                              continue;
                          }
              ...
              if ($key_menu == $uuid) {
                foreach ($childs as $key_child => $child_uuid) {
                  $tmp_col_content->mlid = $child_uuid;
                  //again.. I hope this fixes a bug
                  if(empty($menu_config->menu_config->{$key_menu}->rows_content[0][0])) {
                      continue;
                  }
