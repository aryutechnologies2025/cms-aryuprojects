...
namespace Drupal\sprowt_install;
...
/**
 * Copied from  Drupal\devel_entity_updates\DevelEntityDefinitionUpdateManager
 * Class SprowtEntityDefinitionUpdateManager
 * @package Drupal\sprowt_install
 */
class SprowtEntityDefinitionUpdateManager implements ContainerInjectionInterface
{
    ...
    public function entityNeedsUpdate($entity_type_id) {
        ...
        if(empty($complete_change_list['entity_type_id'])) {
            return false;
        }
    ...
    public function fieldNeedsUpdate($entity_type_id, $field_name) {
        // Ensure this works also on Drupal 8.6 and earlier.
        $reflector = new \ReflectionMethod($this->entityDefinitionUpdateManager, 'getChangeList');
        $reflector->setAccessible(TRUE);
        $complete_change_list = $reflector->invoke($this->entityDefinitionUpdateManager);
        ...
        if(empty($change_list['field_storage_definitions'][$field_name])) {
            return false;
        }
    ...
    public function updateEntity($entity_type_id) {
        if(!$this->entityNeedsUpdate($entity_type_id)) {
            return false;
        }
        // Ensure this works also on Drupal 8.6 and earlier.
        $reflector = new \ReflectionMethod($this->entityDefinitionUpdateManager, 'getChangeList');
    ...
    public function updateField($entity_type_id, $field_name) {
        if(!$this->fieldNeedsUpdate($entity_type_id, $field_name)) {
            return false;
        }
        ...
        $change = $change_list['field_storage_definitions'][$field_name];
